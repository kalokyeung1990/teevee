...
class QualityStringTests(unittest.TestCase):
    ...
    def test_sd_tv(self):
        ...
        for name, tests in self.test_cases.items():
            for test in tests:
                if name == cur_test:
                    assert cur_qual == common.Quality.nameQuality(test)
                else:
                    assert cur_qual != common.Quality.nameQuality(test)
    ...
    def test_hd_tv(self):
        ...
        for name, tests in self.test_cases.items():
            for test in tests:
                if name == cur_test:
                    assert cur_qual == common.Quality.nameQuality(test), test
                else:
                    assert cur_qual != common.Quality.nameQuality(test), test
...
class StatusStringsTests(unittest.TestCase):
    ...
    def test_all(self):
        ...
        for i in valid:
            assert i in status_strings

        for i in unused:
            assert i not in status_strings
...
class OverviewTests(unittest.TestCase):
    ...
    def test_overview_strings(self):
        ...
        assert overview.overviewStrings[overview.SKIPPED] == "skipped"
        assert overview.overviewStrings[overview.WANTED] == "wanted"
        assert overview.overviewStrings[overview.QUAL] == "qual"
        assert overview.overviewStrings[overview.GOOD] == "good"
        assert overview.overviewStrings[overview.UNAIRED] == "unaired"
        assert overview.overviewStrings[overview.SNATCHED] == "snatched"
