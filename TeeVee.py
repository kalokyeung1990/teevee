...
from teevee.helper.common import choose_data_dir
from teevee.init_helpers import check_installed, get_current_version, remove_pid_file, setup_gettext
from teevee.movies import MovieList
from teevee.oldbeard.name_parser.parser import NameParser, ParseResult
...
from teevee.helper.argument_parser import TeeVeeArgumentParser
from teevee.oldbeard import db, name_cache, network_timezones
from teevee.oldbeard.event_queue import Events
from teevee.tv import TVShow
from teevee.update_manager import PipUpdateManager, UpdateManager
from teevee.views.server_settings import SCWebServer
...
class TeeVee:
    ...
    def start(self):
        ...
        args = TeeVeeArgumentParser(settings.DATA_DIR).parse_args()
        ...
        logger.info("Starting TeeVee [{version}] using '{config}'".format(version=get_current_version(), config=settings.CONFIG_FILE))
            ...
            if event == teevee.oldbeard.event_queue.Events.SystemEvent.RESTART:
                ...
                if popen_list and not settings.NO_RESTART:
                    ...
                    if "--nolaunch" not in popen_list:
                        # noinspection SpellCheckingInspection
                        popen_list += ["--nolaunch"]
                    logger.info("Restarting TeeVee with {options}".format(options=popen_list))
    ...
    def force_update(self):
        """
        Forces TeeVee to update to the latest version and exit.
        ...
        self.log("Forcing TeeVee to update using pip...")
        if not PipUpdateManager().update():
            self.log("Failed to force an update", 1)
            return False

        self.log("Successfully updated to the latest pip release. You may now run TeeVee normally")
        return True
    ...
    def install_file(self, file: Path):
        ...
        self.log("Successfully installed to the wheel. You may now run TeeVee normally")
        return True
