...
from teevee.oldbeard import db, network_timezones, ui
...
class ShowUpdater(object):
    ...
    def run(self, force=False):
        ...
        try:
            ...
            for index, provider in teevee.indexer:
                ...
                if last_update:
                    ...
                    while current_check >= last_update:
                        try:
                            TvdbData = teevee.indexer[1].updates(fromTime=current_check - self.seven_days, toTime=current_check)
                            TvdbData.series()
