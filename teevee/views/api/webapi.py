...
from teevee.helper.common import dateFormat, dateTimeFormat, pretty_file_size, sanitize_filename, timeFormat, try_int
from teevee.helper.exceptions import ShowDirectoryNotFoundException
from teevee.helper.quality import get_quality_string
from teevee.init_helpers import get_current_version
from teevee.oldbeard import classes, db, helpers, network_timezones, scdatetime, search_queue, ui
...
from teevee.oldbeard.postProcessor import PROCESS_METHODS
from teevee.show.ComingEpisodes import ComingEpisodes
...
class AbstractStartScheduler(ApiCall):
    ...
    def run(self):
        ...
        if not isinstance(self.scheduler, teevee.oldbeard.scheduler.Scheduler):
...
class CMDSubtitleSearch(ApiCall):
    ...
    def run(self):
        ...
        if new_subtitles:
            new_languages = [teevee.oldbeard.subtitles.name_from_code(code) for code in new_subtitles]
...
class CMDSickChillSearchIndexers(ApiCall):
    ...
    def __init__(self, args, kwargs):
        super().__init__(args, kwargs)
        self.valid_languages = teevee.indexer.lang_dict()
    ...
    def run(self):
        ...
        if self.name and not self.indexerid:  # only name was given
            search_results = teevee.indexer.search_indexers_for_series_name(self.name, self.lang)
            ...
            indexer, result = teevee.indexer.search_indexers_for_series_id(indexerid=self.indexerid, language=self.lang)
            if not indexer:
