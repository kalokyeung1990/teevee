...
from teevee import adba, logger, settings
from teevee.helper import try_int
from teevee.helper.common import episode_num, pretty_file_size
from teevee.helper.exceptions import CantUpdateShowException, NoNFOException, ShowDirectoryNotFoundException
from teevee.oldbeard.blackandwhitelist import BlackAndWhiteList, short_group_names
from teevee.oldbeard.common import cpu_presets, FAILED, IGNORED, Overview, Quality, SKIPPED, SNATCHED_BEST, statusStrings, UNAIRED, WANTED
from teevee.oldbeard.scene_numbering import (
...
from teevee.oldbeard.trakt_api import TraktAPI
from teevee.providers.GenericProvider import GenericProvider
from teevee.show.Show import Show
from teevee.system.Restart import Restart
from teevee.system.Shutdown import Shutdown
from teevee.tv import TVShow
from teevee.update_manager import UpdateManager
...
class Home(WebRoot):
    ...
    def displayShow(self):
        ...
        show_obj.exceptions = teevee.oldbeard.scene_exceptions.get_scene_exceptions(show_obj.indexerid)
    ...
    def sceneExceptions(self):
        show = self.get_query_argument("show")
        exeptions_list = teevee.oldbeard.scene_exceptions.get_all_scene_exceptions(show)
    ...
    def editShow(
        ...
        show_obj.exceptions = teevee.oldbeard.scene_exceptions.get_all_scene_exceptions(show_obj.indexerid)
        ...
        try:
            teevee.oldbeard.scene_exceptions.update_custom_scene_exceptions(show_obj.indexerid, exceptions)
            time.sleep(cpu_presets[settings.CPU_PRESET])
        ...
        if do_update_scene_numbering:
            try:
                teevee.oldbeard.scene_numbering.xem_refresh(show_obj.indexerid, show_obj.indexer)
                time.sleep(cpu_presets[settings.CPU_PRESET])
            except CantUpdateShowException:
    ...
    def manual_search_show_releases(self):
        ...
        # teevee.oldbeard.search.search_providers(
        #     show_object,
    ...
    def manual_snatch_show_release(self):
        ...
        if result:
            provider = teevee.oldbeard.providers.getProviderClass(result.get("provider"))
            if provider.provider_type == GenericProvider.TORRENT:
                result = teevee.oldbeard.classes.TorrentSearchResult.make_result(result)
            elif provider.provider_type == GenericProvider.NZB:
                result = teevee.oldbeard.classes.NZBSearchResult.make_result(result)
            elif provider.provider_type == GenericProvider.NZBDATA:
                result = teevee.oldbeard.classes.NZBDataSearchResult.make_result(result)
            else:
        ...
        if isinstance(result, str):
            teevee.logger.info(_("Could not snatch manually selected result: {result}").format(result=result))
        elif isinstance(result, teevee.oldbeard.classes.SearchResult):
            teevee.oldbeard.search.snatch_episode(result, SNATCHED_BEST)
    ...
    def getManualSearchStatus(self):
        ...
        def getEpisodes(search_thread, search_status):
            ...
            if isinstance(search_thread, teevee.oldbeard.search_queue.ManualSearchQueueItem):
                # noinspection PyProtectedMember
                results.append(
                    {
        ...
        # Finished Searches
        searchstatus = "Finished"
        for searchThread in teevee.oldbeard.search_queue.MANUAL_SEARCH_HISTORY:
            if show and str(searchThread.show.indexerid) != show:
                continue

            if isinstance(searchThread, teevee.oldbeard.search_queue.ManualSearchQueueItem):
    ...
    def playOnKodi(self, show, season, episode, host):
        ...
        teevee.oldbeard.notifiers.kodi_notifier.play_episode(episode_object, host)
        return json.dumps({"result": "success"})
