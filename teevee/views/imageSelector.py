...
from teevee.oldbeard.helpers import make_indexer_session
from teevee.providers.metadata.generic import GenericMetadata
from teevee.show.indexers.handler import ShowIndexer
...
class ImageSelector(Home):
    ...
    def index(self, show=None, imageType="", provider: int = None):
        ...
        show_obj = Show.find(teevee.settings.show_list, int(show))
        if not show_obj:
        ...
        if provider == ShowIndexer.FANART:
            ...
            if "poster" == imageType:
                images = teevee.indexer[provider].series_poster_url(show_obj, multiple=True)
            elif "banner" == imageType:
                images = teevee.indexer[provider].series_banner_url(show_obj, multiple=True)
            elif "fanart" == imageType:
                images = teevee.indexer[provider].series_fanart_url(show_obj, multiple=True)
            else:
