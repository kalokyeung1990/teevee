...
from teevee.oldbeard.helpers import create_https_certificates, generateApiKey
from teevee.views import CalendarHandler, LoginHandler, LogoutHandler, MoviesHandler
from teevee.views.api import ApiHandler, KeyHandler
...
class TeeVeeStaticFileHandler(StaticFileHandler):
    ...
    def make_static_url(cls, settings: Dict[str, Any], path: str, include_version: bool = True) -> str:
        ...
        if "cache/" in path:
            custom_settings["static_path"] = os.path.dirname(teevee.settings.CACHE_DIR)
...
class SCWebServer(threading.Thread):
    def __init__(self, options=None):
        ...
        self.options.setdefault("port", settings.WEB_PORT or 3333)
        ...
        try:
            ...
            if ex.errno == errno.EADDRINUSE:  # Address/port combination already in use
                if settings.LAUNCH_BROWSER and not self.daemon:
                    teevee.start.launchBrowser("https" if settings.ENABLE_HTTPS else "http", self.options["port"], settings.WEB_ROOT)
