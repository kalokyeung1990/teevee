...
from teevee.helper import sanitize_filename, try_int
from teevee.oldbeard import config, db, filters, helpers, ui
from teevee.oldbeard.blackandwhitelist import short_group_names
from teevee.oldbeard.common import Quality
from teevee.oldbeard.trakt_api import TraktAPI
from teevee.oldbeard.traktTrending import trakt_trending
from teevee.show.recommendations.favorites import favorites
from teevee.show.recommendations.imdb import imdb_popular
...
class AddShows(Home):
    ...
    def searchIndexersForShowName(self):
        ...
        for index, indexer_object in teevee.indexer:
        ...
        for index, shows in results.items():
            ...
            final_results.extend(
                {
                    (
                        teevee.indexer[index].name,
                        index,
                        teevee.indexer[index].show_url,
                        show["id"],
                        show["seriesName"],
                        show["firstAired"],
                        teevee.tv.Show.find(settings.show_list, show["id"]) is not None,
                    )
                    for show in shows
                }
            )
        ...
        lang_id = teevee.indexer[indexer].lang_dict[lang]
    ...
    def getTrendingShowImage(self):
        ...
        image_url = teevee.indexer.series_poster_url_by_id(indexer_id)
        if image_url:
    ...
    def addNewShow(
        ...
        **_kwargs,
        ...
        if full_show_path:
            ...
            s = teevee.indexer.series_by_id(indexerid=indexer_id, indexer=indexer, language=indexer_language)
