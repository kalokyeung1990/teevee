...
class LoginHandler(BaseHandler):
    ...
    def post(self, next_=None):
        ...
        if self.get_body_argument("username", None) == settings.WEB_USERNAME and self.get_body_argument("password", None) == settings.WEB_PASSWORD:
            ...
            self.set_secure_cookie("teevee_user", settings.API_KEY, expires_days=(None, 30)[remember_me])
            logger.info(_("User logged into the TeeVee web interface"))
        else:
            logger.warning(_("User attempted a failed login to the TeeVee web interface from IP: ") + self.request.remote_ip)
