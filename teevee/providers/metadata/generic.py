...
from teevee.helper.common import remove_extension, replace_extension, try_int
...
class GenericMetadata(object):
    ...
    def save_thumbnail(self, episode_object):
        ...
        thumb_url = teevee.indexer.episode_image_url(episode_object)
        if not thumb_url:
    ...
    def save_fanart(self, show_obj):
        ...
        fanart_url = teevee.indexer.series_fanart_url(show_obj)
        if not fanart_url:
    ...
    def save_poster(self, show_obj):
        ...
        poster_url = teevee.indexer.series_poster_url(show_obj)
        if not poster_url:
    ...
    def save_banner(self, show_obj):
        ...
        banner_url = teevee.indexer.series_banner_url(show_obj)
        if not banner_url:
    ...
    def save_season_poster(self, show_obj, season):
        ...
        season_poster_url = teevee.indexer.season_poster_url(show_obj, season)
    ...
    def save_season_banner(self, show_obj, season):
        ...
        show_obj: a TVShow object for which to save the season thumbs
        """
        season_banner_url = teevee.indexer.season_banner_url(show_obj, season)
        ...
        try:
            ...
            if epg_url_text:
                ...
                if str(indexer_id) in epg_url and "tvrage" in epg_url:
                    if teevee.indexer.TVRAGE not in teevee.indexer:
                        logger.warning(_("Invalid Indexer ID ({0}), not using metadata file because it has TVRage info").format(indexer_id))
                        return empty_return
                    return indexer_id, show_xml.findtext("title"), teevee.indexer.TVRAGE
                if str(indexer_id) in epg_url and "tvdb" in epg_url:
                    return indexer_id, show_xml.findtext("title"), teevee.indexer.TVDB
