...
class IndexerFavorites(object):
    ...
    def fetch_indexer_favorites(self):
        ...
        indexer_favorites = teevee.indexer.get_indexer_favorites()
        results = []
    ...
    def cache_image(self, indexerid):
        ...
        if not os.path.isfile(full_path):
            helpers.download_file(teevee.indexer.series_poster_url_by_id(indexerid), full_path, session=self.session)

    @staticmethod
    def test_user_key(user, key, indexer):
        return teevee.indexer.indexers[indexer].test_user_key(user, key)
