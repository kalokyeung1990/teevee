...
from teevee.helper import episode_num, pretty_file_size, SUBTITLE_EXTENSIONS
from teevee.helper.common import is_media_file, replace_extension, USER_AGENT
...
def makeDir(path):
    ...
    if not os.path.isdir(path):
        try:
            ...
            teevee.oldbeard.notifiers.synoindex_notifier.addFolder(path)
        except OSError:
            return False
    return True
        ...
        # not Windows, create all missing folders and set permissions
            ...
            for cur_folder in folder_list:
                ...
                try:
                    ...
                    # do the library update for synoindex
                    teevee.oldbeard.notifiers.synoindex_notifier.addFolder(sofar)
                except (OSError, IOError) as error:
...
def rename_ep_file(cur_path, new_path, old_path_length=0):
    ...
    if cur_file_ext[1:] in SUBTITLE_EXTENSIONS:
        ...
        if sublang in teevee.oldbeard.subtitles.subtitle_code_filter():
    ...
    while os.path.isdir(check_empty_dir) and check_empty_dir != keep_dir:
        ...
        if not check_files or (len(check_files) <= len(ignore_items) and all(check_file in ignore_items for check_file in check_files)):
            ...
            try:
                ...
                # do the library update for synoindex
                teevee.oldbeard.notifiers.synoindex_notifier.deleteFolder(check_empty_dir)
...
def create_https_certificates(ssl_cert, ssl_key):
    ...
    try:
        ...
        from teevee.certgen import createCertificate, createCertRequest, createKeyPair, TYPE_RSA
...
def _check_against_names(nameInQuestion, show, season=-1):
    ...
    showNames.extend(teevee.oldbeard.scene_exceptions.get_scene_exceptions(show.indexerid, season=season))
...
def get_show(name, try_indexers=False):
    ...
    try:
        ...
        cache = teevee.oldbeard.name_cache.get_id_from_name(name)
        if cache:
            from_cache = True
        ...
        if not show_object and try_indexers:
            result = teevee.indexer.search_indexers_for_series_id(name=full_sanitizeSceneName(name))[1]
            if result:
                show_object = Show.find(settings.show_list, result.id)

        # try scene exceptions
        if not show_object:
            scene_exceptions = teevee.oldbeard.scene_exceptions.get_scene_exception_by_name_multiple(name)
