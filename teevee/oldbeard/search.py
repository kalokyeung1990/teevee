...
def search_for_needed_episodes():
    ...
    for curShow in show_list:
        if not curShow.paused:
            teevee.oldbeard.name_cache.build_name_cache(curShow)
    ...
    providers = [x for x in teevee.oldbeard.providers.sorted_provider_list(settings.RANDOMIZE_PROVIDERS) if x.is_active and x.enable_daily and x.can_daily]
...
def search_providers(show, episodes, manual=False, downCurQuality=False):
    ...
    teevee.oldbeard.name_cache.build_name_cache(show)

    # noinspection DuplicatedCode
    original_thread_name = threading.current_thread().name

    providers = [
        x for x in teevee.oldbeard.providers.sorted_provider_list(settings.RANDOMIZE_PROVIDERS) if x.is_active and x.can_backlog and x.enable_backlog
    ]
    for curProvider in providers:
        threading.current_thread().name = f"{original_thread_name} :: [{curProvider.name}]"
        curProvider.cache.update_cache()
