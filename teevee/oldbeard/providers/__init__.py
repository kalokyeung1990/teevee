...
from teevee.oldbeard.providers import (
    abnormal,
    alpharatio,
    archetorrent,
    binsearch,
...
from teevee.providers.nzb.NZBProvider import NZBProvider
from teevee.providers.torrent.TorrentProvider import TorrentProvider
...
def getProviderModule(name):
    name = name.lower()
    prefix = "teevee.oldbeard.providers."
...
def check_enabled_providers():
    if not settings.DEVELOPER:
        ...
        if not (daily_enabled and backlog_enabled):
            ...
            teevee.oldbeard.helpers.add_site_message(
                formatted_msg.format(searches=searches, web_root=settings.WEB_ROOT), tag="no_providers_enabled", level="danger"
            )
        else:
            teevee.oldbeard.helpers.remove_site_message(tag="no_providers_enabled")
