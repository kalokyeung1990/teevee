...
from teevee.helper.common import episode_num, sanitize_filename
from teevee.oldbeard.trakt_api import TraktAPI, traktException
...
class TraktChecker(object):
    ...
    def _trakt_knows_show(self, indexer, indexerid):
        ...
        try:
            ...
            trakt_show = [x for x in library if int(indexerid) == int(x["show"]["ids"][teevee.indexer.slug(indexer)] or -1)]
        ...
        if settings.TRAKT_SYNC_REMOVE and settings.TRAKT_SYNC and settings.USE_TRAKT:
            ...
            for cur_episode in main_db_con.select(sql_selection):
                if self._is_in_collection(
                    teevee.indexer.slug(cur_episode["indexer"]),
        ...
        if settings.USE_TRAKT and settings.TRAKT_SYNC_WATCHLIST:
            ...
            for cur_episode in main_db_con.select(sql_selection):
                if self._is_in_epoisode_watchlist(
                    teevee.indexer.slug(cur_episode["indexer"]), str(cur_episode["showid"]), str(cur_episode["season"]), str(cur_episode["episode"])
                ):
    ...
    def _update_shows(self):
        ...
        for index, indexer in teevee.indexer:
    ...
    def _get_shows_watchlist(self):
        ...
        """
        try:
            self.show_watchlist = {indexer.slug: {} for index, indexer in teevee.indexer}
    ...
    def _get_episode_watchlist(self):
        ...
        """
        try:
            self.episode_watchlist = {indexer.slug: {} for index, indexer in teevee.indexer}
    ...
    def _get_shows_collection(self):
        ...
        """
        try:
            self.collection_list = {indexer.slug: {} for index, indexer in teevee.indexer}
    ...
    def _trakt_bulk_data_generate(data):
        ...
        for showid, indexer, show_name, startyear, season, episode in data:
            slug = teevee.indexer.slug(indexer)
