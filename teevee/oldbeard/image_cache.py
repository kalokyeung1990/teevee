...
from teevee.providers.metadata.generic import GenericMetadata
from teevee.providers.metadata.helpers import getShowImage
...
class ImageCache(object):
    ...
    def _cache_image_from_indexer(self, show_obj, img_type):
        ...
        # generate the path based on the type & indexer_id
        if img_type == self.POSTER:
            img_url = teevee.indexer.series_poster_url(show_obj)
            if not img_url:
            ...
            img_url = teevee.indexer.series_banner_url(show_obj)
            if not img_url:
            ...
            img_url = teevee.indexer.series_poster_url(show_obj, thumb=True)
            if not img_url:
            ...
            img_url = teevee.indexer.series_banner_url(show_obj, thumb=True)
            if not img_url:
            ...
            img_url = teevee.indexer.series_fanart_url(show_obj)
            if not img_url:
