...
def set_scene_numbering(indexer_id, indexer, season=None, episode=None, absolute_number=None, sceneSeason=None, sceneEpisode=None, sceneAbsolute=None):
    ...
    if season and episode:
        main_db_con.action(
            "INSERT OR IGNORE INTO scene_numbering (indexer, indexer_id, season, episode) VALUES (?,?,?,?)", [indexer, indexer_id, season, episode]
        )
        ...
        main_db_con.action(
            "INSERT OR IGNORE INTO scene_numbering (indexer, indexer_id, absolute_number) VALUES (?,?,?)", [indexer, indexer_id, absolute_number]
        )
...
def get_indexer_absolute_numbering_for_xem(indexer_id, indexer, sceneAbsoluteNumber, scene_season=None):
    ...
    if scene_season is None:
        rows = main_db_con.select(
            "SELECT absolute_number FROM tv_episodes WHERE indexer = ? and showid = ? and scene_absolute_number = ?", [indexer, indexer_id, sceneAbsoluteNumber]
        )
    else:
        rows = main_db_con.select(
...
def xem_refresh(indexer_id, indexer, force=False):
    ...
    if refresh or force:
        logger.debug(f"Looking up XEM scene mapping for show {indexer_id} on {teevee.indexer.name(indexer)}")
        ...
        try:
            # XEM MAP URL
            url = f"https://thexem.info/map/havemap?origin={teevee.indexer.slug(indexer)}"
            parsed_json = teevee.oldbeard.helpers.getURL(url, session=xem_session, returns="json")
            if (
                not parsed_json
                or "result" not in parsed_json
                or "success" not in parsed_json["result"]
                or "data" not in parsed_json
                or str(indexer_id) not in parsed_json["data"]
            ):
                return

            # XEM API URL
            url = f"https://thexem.info/map/all?id={indexer_id}&origin={teevee.indexer.slug(indexer)}&destination=scene"

            parsed_json = teevee.oldbeard.helpers.getURL(url, session=xem_session, returns="json")
            if not parsed_json or "result" not in parsed_json or "success" not in parsed_json["result"]:
                logger.info(f'No XEM data for show "{indexer_id} on {teevee.indexer.name(indexer)}"')
                return
                    ...
                    cl.append(
                        [
                            ...
                            [
                                ...
                                indexer_id,
                                entry[teevee.indexer.slug(indexer)]["season"],
                                entry[teevee.indexer.slug(indexer)]["episode"],
                            ],
                        ]
                    )
                    cl.append(
                        [
                            ...
                            [
                                entry[teevee.indexer.slug(indexer)]["absolute"],
                                indexer_id,
                                entry[teevee.indexer.slug(indexer)]["season"],
                                entry[teevee.indexer.slug(indexer)]["episode"],
                            ],
            ...
            logger.warning(f"Exception while refreshing XEM data for show {indexer_id} on {teevee.indexer.name(indexer)}: {error}")
...
def fix_xem_numbering(indexer_id, indexer):
    ...
    logger.debug(f"Fixing any XEM scene mapping issues for show {indexer_id} on {teevee.indexer.name(indexer)}")
    ...
    for row in rows:
        ...
        if update_absolute_number:
            cl.append(
                ["UPDATE tv_episodes SET absolute_number = ? WHERE showid = ? AND season = ? AND episode = ?", [absolute_number, indexer_id, season, episode]]
            )
            update_absolute_number = False

        if update_scene_season:
            cl.append(["UPDATE tv_episodes SET scene_season = ? WHERE showid = ? AND season = ? AND episode = ?", [scene_season, indexer_id, season, episode]])
            update_scene_season = False

        if update_scene_episode:
            cl.append(
                ["UPDATE tv_episodes SET scene_episode = ? WHERE showid = ? AND season = ? AND episode = ?", [scene_episode, indexer_id, season, episode]]
            )
            update_scene_episode = False
