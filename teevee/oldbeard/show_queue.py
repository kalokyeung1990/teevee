...
class QueueItemAdd(ShowQueueItem):
    ...
    def run(self):
        ...
        try:
            s = teevee.indexer.series_by_id(indexerid=self.indexer_id, indexer=self.indexer, language=self.lang)
            if not s:
                error_string = _("Could not find show with id:{indexer_id} on {indexer}, skipping").format(
                    indexer_id=self.indexer_id, indexer=teevee.indexer.name(self.indexer)
                )
            ...
            if getattr(s, "seriesName", None) is None:
                ...
                ).format(show_dir=self.show_dir, indexer=teevee.indexer.name(self.indexer))
            ...
            error_string = (
                f"Unable to look up the show in {self.show_dir} on {teevee.indexer.name(self.indexer)} "
            ...
            if settings.USE_TRAKT:
                ...
                data = {"shows": [{"title": title, "ids": {teevee.indexer.slug(self.indexer): self.indexer_id}}]}
        ...
        try:
            ...
            error_string = f"Unable to add {self.show.name if self.show else 'show'} due to an error with {teevee.indexer.name(self.indexer)}"
