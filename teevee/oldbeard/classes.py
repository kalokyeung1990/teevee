...
import teevee
from teevee.helper.common import dateTimeFormat
...
class SearchResult(object):
    ...
    def make_result(cls, result_dict):
        error, show = teevee.show.Show.Show.validate_indexer_id(result_dict.get("indexerid"))
        if not show:
            return error

        episode_objects = [show.get_episode(result_dict.get("season"), ep) for ep in result_dict.get("episodes").split("|") if ep]
        provider = teevee.oldbeard.providers.getProviderClass(result_dict.get("provider"))
...
class Proper(object):
    ...
    def __str__(self):
        return "{date} {name} {season}x{episode} of {series_id} from {indexer}".format(
            date=self.date, name=self.name, season=self.season, episode=self.episode, series_id=self.indexerid, indexer=teevee.indexer.name(self.indexer)
        )
