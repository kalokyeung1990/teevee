...
import teevee.oldbeard.scene_numbering
from teevee import logger, settings
from teevee.helper.common import dateTimeFormat, episode_num, is_media_file, remove_extension, replace_extension, sanitize_filename, try_int
from teevee.helper.exceptions import (
...
from teevee.oldbeard import db, helpers, network_timezones, notifiers, postProcessor, subtitles
from teevee.oldbeard.blackandwhitelist import BlackAndWhiteList
from teevee.oldbeard.common import (
    ARCHIVED,
    DOWNLOADED,
    FAILED,
    IGNORED,
...
from teevee.oldbeard.name_parser.parser import InvalidNameException, InvalidShowException, NameParser
...
class TVShow(object):
    ...
    @property
    def idxr(self):
        return teevee.indexer[self.indexer]
    ...
    def load_from_indexer(self):
        ...
        indexer_show = teevee.indexer.series(self)
    ...
    def load_imdb_info(self):
        # TODO: Create shows only database from s3 datasets, possibly distributable from teevee.github.io until they are integrated into sickindexer
    ...
    def delete_show(self, full=False):
        ...
        for provider in teevee.oldbeard.providers.__all__:
...
class TVEpisode(object):
    ...
    def load_from_db(self, season, episode):
        ...
        if len(sql_results) > 1:
            ...
            teevee.oldbeard.scene_numbering.xem_refresh(self.show.indexerid, self.show.indexer)
            ...
            if self.scene_absolute_number == 0:
                self.scene_absolute_number = teevee.oldbeard.scene_numbering.get_scene_absolute_numbering(
                    self.show.indexerid, self.show.indexer, self.absolute_number
                )

            if self.scene_season == 0 or self.scene_episode == 0:
                self.scene_season, self.scene_episode = teevee.oldbeard.scene_numbering.get_scene_numbering(
